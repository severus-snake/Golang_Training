...
func main() {
	...
	for {  //main loop starts here
		...
		switch getInput() {  //NOTE: this is a CALL to the getInput() function. It returns the command typed in by the user as a string
			...
			case inputUp:movePlayer(up)
			case inputLeft:movePlayer(left)
			case inputDown:movePlayer(down)
			case inputRight:movePlayer(right)
			//shooting starts here
			case inputShootUp:shoot(up)
			case inputShootLeft:shoot(left)
			case inputShootDown:shoot(down)
			case inputShootRight:shoot(right)
			case inputQuit:
